...
class Player:
    def __init__(self):
        ...
        self.name: str = ""
        self.pos: str = ""
        self.bid: str = ""            # Startbid in paren if no bids.    
...
def get_current_date(soup: BeautifulSoup) -> list:
    ...
    try:
        ...
        print(f"Paste HTML content in {FILE_ROSTER} or {FILE_TRANSFER}")
        exit()
...
def print_value_predictions(players: list[Player]) -> None:
    ...
    for p in players:
        ...
        print(headline)
        print(f"Värde:  {num2str(p.value)} kr")
        if player.age == 17:
            # Players over the age of 17 rarely develop at 300k/w
            print(f"300k/w: {num2str(p.value + rem_trainings * 300000)} kr")

        print(f"400k/w: {num2str(p.value + rem_trainings * 400000)} kr")
        print(f"500k/w: {num2str(p.value + rem_trainings * 500000)} kr")

        if p.age > 17:
            print(f"600k/w: {num2str(p.value + rem_trainings * 600000)} kr")
